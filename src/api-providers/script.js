import {
  providerRatings,
  providerRatingsComment,
  ownerInfo,
} from "./modules/providerInfo.js";
...
const fetchWithRetry = async (url, maxRetries = 1, delay = 1000) => {
  let lastError;
...
function disableAllButtons(disabled) {
  document
    .querySelectorAll("button, .api-button, .view-button")
    .forEach((button) => {
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    if (
      !elements.modelContainer ||
      !elements.loadingElement ||
      !elements.modelCountElement ||
      !providerInfoBox ||
...
async function fetchAllModels() {
  ...
  const fetchResults = await Promise.all(
    providers.map((provider) =>
      fetchProviderData(provider, results, errorMessages)
    )
  );
  ...
  if (!results.rimunace || results.rimunace.length === 0) {
    try {
      const response = await fetchWithRetry(
        "/.netlify/functions/api-handler/rimunace"
      );
  ...
  const failedProviders = Object.values(results).filter(
    (arr) => arr.length === 0
  ).length;
...
async function fetchProviderData(provider, results, errorMessages) {
  try {
    const response = await fetchWithRetry(
      `/.netlify/functions/api-handler/${provider}`
    );
...
document
  .querySelector(".api-buttons")
  .addEventListener("click", handleApiButtonClick);
...
async function updateProviderUI(provider) {
  ...
  apiDescription.textContent =
    apiDescriptions[provider] || "No description available";
}
    ...
    if (status.connected) {
      ...
      redisStatus.innerHTML = `
                ...
                <div class="cache-details">
                    ${
                      lastSaveTime
                        ? `Last Update: ${formatDate(lastSaveTime)}<br>`
                        : ""
                    }
...
function createProviderButton(provider) {
  return `
        <button class="api-button" data-api="${provider}">
            <img src="${getProviderIconPath(
              provider
            )}" alt="${provider}" class="provider-icon">
            ${provider}
        </button>
    `;
}
